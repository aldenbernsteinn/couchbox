...
// D-pad → arrow keys, RT → Enter, LT → Esc, Back → Super (or voice when kb open).
...
// Mouse mode settings
const DEADZONE = 4000;
const STICKY_DEADZONE = 12000;
const MOUSE_SPEED = 18;
const SCROLL_SPEED = 3;
const MOUSE_TICK_MS = 16;
const CURSOR_POLL_MS = 150;
const IDLE_TIMEOUT_MS = 60000;
const TRIGGER_THRESHOLD = 16000;
...
// Mouse mode state
let stickX = 0, stickY = 0, rStickX = 0, rStickY = 0;
...
// Trigger/dpad state
let ltFired = false, rtFired = false;
let dpadLastX = 0, dpadLastY = 0;
...
function mouseMoveTick() {
  ...
  if (sy !== 0) {
    ...
    const lines = Math.max(1, Math.round(Math.abs(sy) * SCROLL_SPEED));
    for (let i = 0; i < lines; i++) execFile('xdotool', ['click', button], () => {});
  }
  const sx = applyDeadzone(rStickX, DEADZONE);
  if (sx !== 0) {
    const button = sx < 0 ? '6' : '7';
    const lines = Math.max(1, Math.round(Math.abs(sx) * SCROLL_SPEED));
    for (let i = 0; i < lines; i++) execFile('xdotool', ['click', button], () => {});
  }
}
...
function startMouseMode() {
  ...
  ltFired = false; rtFired = false;
  dpadLastX = 0; dpadLastY = 0;
...
function onAButton(pressed) {
  ...
  aButtonDown = pressed;
  if (pressed) {
    execFile('xdotool', ['mousedown', '1'], () => { checkForTextField(); });
  } else {
    execFile('xdotool', ['mouseup', '1'], () => {});
  }
}
...
function launchKeyboard() {
  ...
  keyboardProc.stderr.on('data', (d) => {
    const msg = d.toString().trim();
    if (msg) console.log(`[keyboard] ${msg}`);
  });
...
// ── LB / RB — move text cursor when kb open ─────────────────────────
...
// ── Triggers (LT → Esc, RT → Enter — RT closes kb when open) ───────
    ...
    const readLoop = () => {
      ...
      fs.read(fd, buf, 0, JS_EVENT_SIZE, null, (err, bytesRead) => {
        ...
        if (realType === JS_EVENT_BUTTON && !isInit) {
          if (number === GUIDE_BUTTON) onGuideButton(value === 1);
          else if (number === BUTTON_A) onAButton(value === 1);
          else if (number === BUTTON_B) onBButton(value === 1);
          else if (number === BUTTON_X) onXButton(value === 1);
          else if (number === BUTTON_Y) onYButton(value === 1);
          else if (number === BUTTON_LB) onLBButton(value === 1);
          else if (number === BUTTON_RB) onRBButton(value === 1);
          else if (number === BUTTON_BACK) onBackButton(value === 1);
        } else if (realType === JS_EVENT_AXIS && !isInit) {
          if (number === AXIS_LEFT_X) {
            stickX = value;
            if (!mouseModeActive && !electronProc && Math.abs(value) > DEADZONE) startMouseMode();
            ...
            rStickY = value;
            if (mouseModeActive) resetIdleTimer();
          } else if (number === AXIS_DPAD_X) { onDpadX(value); }
          else if (number === AXIS_DPAD_Y) { onDpadY(value); }
          else if (number === AXIS_LT) { onLeftTrigger(value); }
          else if (number === AXIS_RT) { onRightTrigger(value); }
        }
        readLoop();
      });
    };
